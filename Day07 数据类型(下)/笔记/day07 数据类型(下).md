...
v2 = set(v1)
print(v2) # {11,22,33,3,99} 重复值自动去除,此功能用的较多
```
...
- ```python
  v1 = set()
  v1.add("武沛齐")
  #第一步,利用哈希函数将"武沛齐"转化为一个数值,例如894234224544
  v1 = hash()
  #第二步取余数(实际掩码做&运算)
  894234224544 % 7 = 3
  #第三步 将"武沛齐"放在哈希表的3的索引位置
  ```
...
总结：集合的**元素**只能是 int、bool、str、tuple 。
...
- 效率高 (集合效率高) 
  ...
  ```python
  #存储原理导致的, 根据2步查找位置,查看位置上是否有信息,直接判断,无须将整个集合进行循环
...
| set   | 是       | 否       | 可哈希   | 否         | set(其他) #元素必须可哈希 | `v=set()`         |
...
v2 = None
..
v1 = [11,22,33,44] #新的v1指向自己的区域
v2 = [111,22,43]   #新的v2指向自己的区域
```
...
v4 = []  #v4指向自己的区域
...
v3 = [11,22,33,44]  #新的v3指向自己的区域
v4 = [111,22,43]    #新的v4指向自己的区域  分别不同的区域,站空间
```
...
print(data_dict) # {1: 5} 1和True相等,第一个被覆盖掉
```
...
1. 获取值
   ...
   data = info.get("email") # 在上面info没有data这个键的时候,键不存在，默认返回 None // print(data) = None
   """
   #如果info里面有 "data":None 这个信息,那么判断就有问题,可以按照下面的办法
   if data == None:
       print("此键不存在")
   else:
       print(data)
...
5. 设置值
   ...
   data.setdefault("name", "alex")  #键存在,不设置也不覆盖
   print(data)  # {'name': '武沛齐', 'email': 'xxx@live.com', 'age': 18}
   ```
...
5. 根据键 修改值 和 添加值 和 删除键值对
   ...
   # 12
   #结果是上面,pop不仅可以删去键,还可以获取值,del是直接删去
   ```
