...
- 情景2
  ...
  execute()
  #你好
  666 # 跳过execute,直接把func重新定义为666,再继续执行execute()
  开始
  666
  结束
...
1. 分析代码，写结果
   ...
   def func():
       def inner():
           print(name) # 武沛齐, 但是inner()无返回值,所有是None,res赋值为None,然后func()函数 return res
       res = inner()
       return res
...
5. 分析代码，写结果
   ...
   v1 = func('武沛齐')
   v2 = func('alex')
   v1() # 武沛齐  没有luffy这个返回值,因为没有print(v1),没有接受返回值
   v2() # alex  没有luffy....
   ```
...
def auth(func):
    @functools.wraps(func)   # inner.__name__ = func.__name__ doc也同样
    def inner(*args, **kwargs):
        return func(*args, **kwargs)
    return inner
