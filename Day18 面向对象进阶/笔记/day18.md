...
# 绑定方法（主流 对象调用）
obj = Foo("武沛齐",20)
obj.f1() #  通过对象调用,参数自动传输 ;
Foo.f1(obj) # 通过类调用Foo.f1(obj),但是参数要手动输入,一般不用
# 绑定方法 武沛齐
# 绑定方法 武沛齐

# 类方法(主流 类调用) 无论什么方式调用,cls始终都是类
Foo.f2()  # cls就是当前调用这个方法的类。（类） 类调用
obj.f2()  # cls就是当前调用这个方法的对象的类。 对象调用
...
class Download(object):
    ...
    def download_dou_yin():
        ...
        with open("xxx.mp4", mode='wb') as f: # 此时用不到self参数,只是简单地直接写路径等信息
            f.write(res.content)

    def download_dou_yin_2(self):
        # 下载抖音
        res = requests.get('.....')
        path = os.path.join(self.folder_path, 'xxx.mp4') # 用到self参数了
...
```python
@classmethod  类方法,有默认参数cls
@staticmethod 静态方法,无参数
如果只是简单的逻辑,没有用到参数,可以直接用@staticmethod 静态方法, 如果用到cls参数,建议写@classmethod 类方法
```
...
v2 = obj.f2  # 绑定方法 + 特殊装饰器 组合创造出来属性,无须加()
print(v2)
```
...
while True:
    ...
    page_data_list = user_list[ pg_object.start() : pg_object.end() ] # 原始需要加括号
...
while True:
    ...
    pg_object = Pagination(page, 20)
    page_data_list = user_list[ pg_object.start : pg_object.end ] # 用了@property之后无()
...
- 方式一，基于装饰器
  ...
  class C(object):
      ...
      @x.setter # 方法.setter  obj.x = 123
      def x(self, value):
          pass
      
      @x.deleter # 方法.deleter  del obj.x
      def x(self):
  		pass
  ...
  obj.x
  obj.x = 123 # 自动触发 方法.setter 调用和装饰器一一对应
  del obj.x  # 自动触发 方法.deleter 调用和装饰器一一对应
  ```
...
- 方式二，基于定义变量
  ...
  class C(object):
      ...
      x = property(getx, setx, delx, "I'm the 'x' property.") # 定义
                                            # 注释
          #如果只想让obj.x即getx使用, 后面可以写None
      x = property(getx, None, None, "I'm the 'x' property.")    # 后两种写法不支持   
...
v2 = obj.get_data() # 曲线使用,get_data是共有的,里面的return是可以被调用的
print(v2)
```
...
obj = Foo()
print(obj.age)
print(obj._Foo__num) # 一个_+类名+私有对象
obj._Foo__msg() # 一个_+类名+私有方法
```
...
- `__enter__`、`__exit__` (成对出现)
  ...
  with SqlHelper() as 连接: # 把对象()放在with里面自动触发__enter__, as后面就是enter return的值
      连接.操作..
...
- `__add__` 等。(减,乘,平方等都可以有相应的方法表示,但是不常用)
...
- `__iter__`
  ...
  - 生成器
    ...
    如果按照迭代器的规定来看，其实生成器类也是一种特殊的迭代器类（生成器也是一个种特殊的迭代器）。
    ```
...
6. 看代码写结果
   ...
   print(obj1.num) #666
   print(obj1.a1) # 1 对象obj1 = Foo(666)里面没有a1,所有去类里面找
   
   obj1.num = 18
   obj1.a1 = 99 # 只是给对象里面赋值了一个a1 = 99,不会改变类里面的a1
...
7. 看代码写结果，注意返回值(函数或者方法或是没有返回值,默认返回None)。
...
8. 看代码写结果【如果有错误，则标注错误即可，并且假设程序报错可以继续执行】
   ...
   Foo.f1() # f1 >>> 正确结果: 报错,绑定方法需要手动传参,但是这个无参数
   Foo.f2() # f2
   ```
...
11. 看代码写结果【如果有错误，则标注错误即可，并且假设程序报错可以继续执行】
    ...
    print(obj.__a2) # 2 >>> 正确答案 报错
    print(Foo.a1) # 1
    print(Foo.__a2) # 2 >>> 正确答案 报错
    obj.show_data() # 667
    ```
...
13. 看代码写结果
    ...
    obj_list = [Base(1), Foo(2), Foo(3)] # Foo里面没有__init__需要去父级找,三个分别封装了a1=1,a1=2,a1=3
...
17. 看代码写结果
    ...
    '''
    site._registry = {
    	'武沛齐': StarkConfig(19)对象(里面存储num=19),
    	'root': StarkConfig(20)对象(里面存储num=20),
    	'admin': RoleConfig(33)对象(里面存储num=33),
    }
    '''
