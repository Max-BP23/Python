...
# multiprocessing.set_start_method('fork') 把mac的系统设置为fork
# 进程创建之后，在进程中还会创建一个线程。
...
if __name__ == '__main__': # 要放在main下面循环,因为系统不一样,看下面系统支持说明
    print(time.time())
    # Linux系统fork；win:spawn ; mac支持：fork和spawn（python3.8默认设置spawn,也可以改为fork,看上面）。
    for name, url in url_list:
...
- `t.start()`，当前线程准备就绪（等待CPU调度，具体时间是由CPU来决定）。
  ...
  t.start() # 到底什么时间开始,不知道,下面的print(number)可能是任何值
...
- `t.join()`，等待当前线程的任务执行完毕后再向下继续执行。
  ...
  print(number) # 不确定,分片操作,可能会造成数据紊乱
  ```
...
- 自定义线程类，直接将线程需要做的事写到run方法中。
  ...
  class MyThread(threading.Thread):
      def run(self):
          print('执行此线程', self._args) # 100, 传进来的参数
...
可以手动加锁&释放锁, 也可以基于上下文管理 with 然后自动包含加锁和释放锁
...
- 示例1
  ...
  print(number) # 结果不确定,数据紊乱,手动加锁可以解决
  ```
  ...
  def _add(count):
      ...
      for i in range(count):
          number += 1
      lock_object.release() # 释放锁, 然后等待的才可以申请到锁
...
def task():
    print("开始")
    lock_object.acquire() # 锁一次
    lock_object.acquire() # 再锁一次
    print(123)
    lock_object.release() # 释放一次
    lock_object.release() # 再次释放
...
虽然lock的锁一次释放一次效率比rlock高,但是Rlock支持嵌套,一般写程序都用Rlock,方便不同同事协同编写代码
...
def task1():
    lock_1.acquire() # 获取锁1
    time.sleep(1)
    lock_2.acquire() # 想要获取锁2,但是锁2已经被task2获取了,也没释放,此时就陷入僵局,死锁
    print(11)
    lock_2.release()
    print(111)
    lock_1.release()
...
```python
import time
from concurrent.futures import ThreadPoolExecutor # 导入这个模块

# pool = ThreadPoolExecutor(100) 表示维护了多少线程
...
for url in url_list:
    # 在线程池中提交一个任务，线程池中如果有空闲线程，则分配一个线程去执行，执行完毕后再将线程交还给线程池；如果没有空闲线程，则等待。
    future = pool.submit(task, url) # 交给线程池之后,返回一个特殊对象(不是结果),在调用下面的done函数
...
for url in url_list:
    # 在线程池中提交一个任务，线程池中如果有空闲线程，则分配一个线程去执行，执行完毕后再将线程交还给线程池；如果没有空闲线程，则等待。
    future = pool.submit(task, url)  # 交给线程池之后,返回一个特殊对象
    future_list.append(future) # 放在之前建的列表中
    
pool.shutdown(True) # 执行完毕
for fu in future_list:
    print(fu.result()) # 拿到所有的结果
```
...
def download(file_name, image_url): # 如果闭包,里面的file_name就没有必要有了
    res = requests.get(
        url=image_url,
        headers={
...
- 多线程执行单例模式，有BUG
  ...
  class Singleton:
      ...
      def __new__(cls, *args, **kwargs):
          if cls.instance:
              return cls.instance
          time.sleep(0.1) # 都停滞在上面,得出结果的就不一样,是bug
          cls.instance = object.__new__(cls)
          return cls.instance
...
5. 程序从flag a执行到falg b的时间大致是多少秒？
   ...
   def _wait():
   	time.sleep(60)
   # flag a
   t = threading.Thread(target=_wait) # 主线程结束整个都结束了,不会等
   t.setDaemon(True)
   t.start()
   # flag b
   ```
...
6. 程序从flag a执行到falg b的时间大致是多少秒？
   ...
   def _wait():
   	time.sleep(60)
   # flag a
   t = threading.Thread(target=_wait) # 需要等
   t.start()
   t.join()
   # flag b
   ```
...
7. 读程序，请确认执行到最后number是否一定为0
   ...
   ```python
   import threading
   loop = int(1E7) # 科学计数法 1*10^7 (10的七次方)
   def _add(loop = 1):
   	global number
   	for _ in range(loop):
   		number += 1
...
9. data.txt 文件中共有 10000 条数据，请为每 100行 数据创建一个线程，并在线程中把当前100条数据的num列相加并打印。
   ...
   def task(row_list):
       num_list = [int(row.split(",")[-1]) for row in row_list] # 推导式,最后一个索引,int下
       result = sum(num_list)
       print(result)
   ...
   def run():
       ...
       file_object.readline() # 读一行, 也没要,后续就从第二行还是读
       row_list = []
       for line in file_object: # 从第二行还是
           row_list.append(line.strip()) # 如果列表长度到了100
           if len(row_list) == 100:
               t = threading.Thread(target=task, args=(row_list,))
               t.start()
               row_list = []  # 清空row_list,直接为空列表,新的内存地址, 注意!!!!但是千万不要用 row_list.clear(),因为实际指向的是内存地址,其他函数也是指向内存地址,如果清空之后,线程里面也清空了
       if row_list: # 万一最后一次是99不到100, 此处可以将row_list的所有数据相加下
           t = threading.Thread(target=task, args=(row_list,))
           t.start()
