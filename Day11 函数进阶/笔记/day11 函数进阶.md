...
- 坑
  ...
  # a1=99
  # a2 -> 1111111101
  func(99,[77,88]) # 99 [77,88,666] # 因为a2这次传值了
...
res = data_list[2]()
print( res ) # 执行函数 func，并获取返回值；print再输出返回值。 # 123

print( data_list[2]() ) # 123
```

注意：函数同时也可被哈希，所以函数名同时也可以当做 集合的元素、字典的键。
...
- 情景1，例如：要开发一个类似于微信的功能。
  ...
  function_dict = {
      "1": send_message, # 函数名,不是函数,如果是函数,会直接执行
      "2": send_image,
      "3": send_emoji,
      "4": send_file,
      "5": xxx
...
if not item:
    ...
    param_list = item[1] #  ['15131255089', '你好呀'] 参数列表
...
- 将函数名赋值给其他变量，函数名其实就个变量，代指某函数；如果将函数名赋值给另外一个变量，则此变量也会代指该函数，例如：
  ...
  func(11,22) # 也可以直接执行,只是说可以赋值,变量有相同功能
  func_list[0](11,22)
  func_list[1](33,44)
  func_list[2](55,66)
  ```
...
-  return prev,content # 返回两个值且逗号拼接,返回的是元组
...
def func():
    ...
    for num in range(10):
        print(num)
    print(num) # 不会报错,因为在这个区域内已经有了num这个变量
