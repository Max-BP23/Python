...
- 在文件夹下面创建一个py文件,命名为#__int__.py ,然后注释下这个文件是什么作用
...
key_list = list(info.keys()) #一个变量就要识别一遍,效率低
...
可以基于Pycharm的格式化工具来实现自动处理,pycharm - code - reformat code(快捷键ctrl+alt+L)。
...
- `>>`，右移动
  ...
  ```python
  a = 60            # 60 = 0011 1101  向右移动2位,把最后面的2位值删去
  c = a >> 2;       # 15 = 0000 1111
  ```
