...
alter table 表 change id id int not null; -- 先把自增去掉,然后修改去掉自增和主键即可
```
...
当然，MySQL中也支持让你去自定义函数。(开发过程中几乎用不到,专业搞MySQL的才会用到)
...
result = cursor.fetchall()
# {"@_p2_0":1,"@_p2_1":22,"@_p2_2":3,"@_p2_3":4 }
...
table = cursor.fetchall() # 得到执行存储过程的结果集
...
BEGIN 
  DECLARE exit handler for sqlexception -- 定义异常
  BEGIN 
    -- ERROR 
    set p_return_code = 1; 
    rollback; -- 把失败的数据给复原回来
  END; 
 
  DECLARE exit handler for sqlwarning  -- 定义警告
  BEGIN 
    -- WARNING 
    set p_return_code = 2; 
    rollback; 
  ...
  COMMIT;  -- 提交事务,如果成功往下走,如果失败往上走,显示异常或者警告
...
begin 
    ...
    declare done int default false; -- 没有布尔类型,通过int来设置


    declare my_cursor CURSOR FOR select id,name from d1;
    
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE; -- 循环完了,找不到其他值,将done变为True
...
- 使用视图,把视图当做一张表(临时表,不可以插入删除数据,只能查看)
