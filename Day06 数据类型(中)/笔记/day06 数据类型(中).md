...
8. 列表元素排序
   ...
   ​			布尔值和整型可以一起进行对比比较(True=1,False=0)
...
5. 索引，一个元素的操作
   ...
   ele = user_list.pop(1) #可以得到删除的值,del不可以
   ```
...
```python
# 面试题
1. 比较值 v1 = (1) 和 v2 = 1 和 v3 = (1,) 有什么区别？ #v1=v2 都是整型, v3是元祖
2. 比较值 v1 = ( (1),(2),(3) ) 和 v2 = ( (1,) , (2,) , (3,),) 有什么区别？
            #v1结果是(1,2,3)             v2里面是元祖, 大元组里面套了3个小元祖
```
...
3. 获取长度
   ...
   ```python
   user_list = ("范德彪","刘华强",'尼古拉斯赵四',)
   print( len(user_list) ) #计算元祖里面元素的个数
   ```
...
6. 步长
   ...
   ```python
   #索引,切片,步长都是只读,且都是生成一份新的数据,原始数据没有变化
   # 字符串 & 元组。 只能有一种翻转方式
   user_list = ("范德彪","刘华强",'尼古拉斯赵四',"宋小宝","刘能")
   data = user_list[::-1]  
...
# 3.请将 88 修改为 87
li[1][2][0] = 87 # (报错，) 不可以修改元组里面的元素

# 4.请将 "wenzhou" 删除，然后再在列表第0个索引位置插入 "周周"
# li.remove("wenzhou") #方法一
# del li[-1]   #方法二 还有pop也可以删除
li.insert(0,"周周")
```
...
is_success = False #默认登录没成功,设置一个变量

for item in user_list:
    # item = ("alex","132")   ("admin","123")    ("eric","123")
    if username == item[0] and password == item[1]:
        is_success = True #等到登录成功了,把他变为True,如果没有登录成功,这个环节永远不会执行
        break
