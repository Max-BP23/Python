...
# handler_map = {user: User_Handler, article: Article_Handler, 3: 1, 4: 1}
...
class User_views():
    ...
    def register(self):
        handler.User_Handler.c("参数")  # cls 类方法,get后无须实例化,因为self无须传参,优化get(user),从其他文件传入
        # handler_map.get(artical)().c("参数")
    # 1.接受用户输入
    # 2.校验用户输入,邮箱,手机号
    # 3.当数据都是干净的,判断用户是否存在
    # 4.用户创建
